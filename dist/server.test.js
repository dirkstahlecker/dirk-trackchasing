...
it('returns track list without configurations', async () => {
    const trackList = await server.getTrackListNoConfigurations();
...
it('getEventStringsForTrack', async () => {
    let info = await server.getEventStringsForTrack(Types_1.TrackName.parse("Seekonk Speedway"));
    expect(info.length).toEqual(46);
    expect(info[0]).toEqual("7-13-16: US Pro stock nationals, INEX legends, pro 4 modifieds");
    info = await server.getEventStringsForTrack(Types_1.TrackName.parse("Pocatello Speedway"));
    expect(info.length).toEqual(6);
    info = await server.getEventStringsForTrack(Types_1.TrackName.parse("Pocatello Speedway (Inner Dirt Oval)"));
    ...
    info = await server.getEventStringsForTrack(Types_1.TrackName.parse("Thompson Speedway - Rallycross"));
    ...
    info = await server.getEventStringsForTrack(Types_1.TrackName.parse("New Hampshire Motor Speedway (Asphalt Legends Oval)"));
    expect(info.length).toEqual(1);
...
it('number of flips per track', async () => {
    let flips = await server.getFlipsForTrack(Types_1.TrackName.parse("Eldora Speedway"));
    expect(flips.length).toBe(3);
    flips = await server.getFlipsForTrack(Types_1.TrackName.parse("Bridgeport Motorsports Park"));
    expect(flips.length).toBe(7);
    flips = await server.getFlipsForTrack(Types_1.TrackName.parse("Gateway Dirt Nationals"));
    expect(flips.length).toBe(17);
    flips = await server.getFlipsForTrack(Types_1.TrackName.parse("Pocatello Speedway"));
    expect(flips.length).toBe(0);
    flips = await server.getFlipsForTrack(Types_1.TrackName.parse("Pocatello Speedway (Inner Dirt Oval)"));
    expect(flips.length).toBe(1);
    flips = await server.getFlipsForTrack(Types_1.TrackName.parse("Atomic Motor Raceway"));
    expect(flips.length).toBe(1);
    flips = await server.getFlipsForTrack(Types_1.TrackName.parse("Lucas Oil Speedway Off Road Course"));
    expect(flips.length).toBe(4);
    flips = await server.getFlipsForTrack(Types_1.TrackName.parse("Texas Motor Speedway")); //only a flip at the configuration
    expect(flips.length).toEqual(0);
    flips = await server.getFlipsForTrack(Types_1.TrackName.parse("Texas Motor Speedway (Asphalt Road Course)"));
    expect(flips.length).toEqual(1);
});
it('flip objects', async () => {
    let flips = await server.getFlipsForTrack(Types_1.TrackName.parse("Pocatello Speedway (Inner Dirt Oval"));
    expect(flips.length).toBe(1);
    let flip = flips[0];
    ...
    flips = await server.getFlipsForTrack(Types_1.TrackName.parse("Knoxville Raceway"));
    flip = flips.find((f) => {
    ...
    flips = await server.getFlipsForTrack(Types_1.TrackName.parse("Lincoln Speedway"));
    flip = flips.find((f) => {
...
it('returns enriched event info', async () => {
    let eventInfo = await server.getEnrichedEventInfoForDate(Types_1.TrackName.parse("Bridgeport Motorsports Park"), "11-08-20");
    ...
    eventInfo = await server.getEnrichedEventInfoForDate(Types_1.TrackName.parse("Kokomo Speedway"), "8-27-20");
    ...
    eventInfo = await server.getEnrichedEventInfoForDate(Types_1.TrackName.parse("Texas Motor Speedway (Asphalt Road Course"), "6-9-18");
...
it('returns all enriched event infos for a track', async () => {
    let eventInfos = await server.getAllEnrichedEventInfosForTrack(Types_1.TrackName.parse("Bridgeport Motorsports Park"));
